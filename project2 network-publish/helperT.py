...
class FaceDataset(Dataset):
    def __init__(self, data_dir, data_type, img_size=224, augment=False, age_stddev=1.0):
        ...
        csv_path = os.path.join(data_dir, f"gt_avg_{data_type}.csv")
        ...
        for _, row in df.iterrows():
            ...
            img_path = os.path.join(img_dir, img_name + "_face.jpg")
    ...
    def __getitem__(self, idx):
        ...
        if self.data_type != "test":
            return np.clip(round(age), 0, 100), transforms(img)
        else:
            return np.random.rand(1)*100, transforms(img)
